...
int cardEffect(int card, int choice1, int choice2, int choice3,
  ...
  switch (card) {
  case adventurer:
    return cardEffect_Adventurer(&drawntreasure, state, &currentPlayer,
                                 &cardDrawn, temphand, &z);

  case council_room:
    return cardEffect_CouncilRoom(&currentPlayer, state, &handPos);

  case feast:
    return cardEffect_Feast(&currentPlayer, state, &handPos, temphand,
                            &choice1, &x);
  ...
  case smithy:
    return cardEffect_Smithy(&currentPlayer, state, &handPos);
